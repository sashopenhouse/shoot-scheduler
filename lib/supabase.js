...
const PROJECT_TYPES = ['windows', 'bathroom', 'doors', 'siding', 'other'];
...
function newShootRow(input) {
  ...
  return {
    ...
    notes: input.notes || '',
    project_type: PROJECT_TYPES.includes(input.projectType) ? input.projectType : null,
...
async function updateShoot(id, patch) {
  ...
  // A changed address invalidates any coordinates geocoded for the old one.
  if (patch.location !== undefined && patch.location !== existing.location) {
    update.lat = null;
    update.lng = null;
...
/** Persists geocoded coordinates for a shoot without touching updated_at. */
async function setShootGeocode(id, { lat, lng }) {
...
/** Shoots that have a location but have never been geocoded (or failed to). */
async function listUngeocodedShoots() {
...
/** Key used for both writing and checking dismissals — normalizes case/whitespace. */
function dismissKeyFor(title) {
  return (title || '').trim().toLowerCase();
}

/** Permanently hides a Connecteam job (by title) from the Jobs list and map. */
async function dismissJob(title) {
  const key = dismissKeyFor(title);
  if (!key) return;
  const { error } = await supabase.from('dismissed_jobs').upsert({ title_key: key, title: title.trim() });
  if (error) throw error;
}

async function undismissJob(title) {
  const { error } = await supabase.from('dismissed_jobs').delete().eq('title_key', dismissKeyFor(title));
  if (error) throw error;
}

async function listDismissedJobs() {
  const { data, error } = await supabase.from('dismissed_jobs').select('*').order('dismissed_at', { ascending: false });
  if (error) throw error;
  return data;
}

/** Set of dismissed title keys, for filtering a list of Connecteam jobs. */
async function dismissedJobKeys() {
  const { data, error } = await supabase.from('dismissed_jobs').select('title_key');
  if (error) throw error;
  return new Set(data.map((row) => row.title_key));
}
