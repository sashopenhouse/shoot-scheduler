...
/**
 * Best-effort project type guess from a job's title/notes text — window,
 * bathroom, door, or siding keywords, checked against New York Sash's own
 * specialties (per the brand guide). Real Connecteam data shows roughly
 * half of jobs have no usable text at all, so this is a starting point the
 * user corrects via a dropdown, never treated as ground truth.
 */
const PROJECT_TYPE_PATTERNS = [
  ['bathroom', /\b(bath(room)?|shower|tub|walk-?in tub)\b/i],
  ['windows', /\bwindows?\b/i],
  ['doors', /\bdoors?\b/i],
  ['siding', /\bsiding\b/i],
];

function guessProjectType(...texts) {
  const combined = texts.filter(Boolean).join(' ');
  if (!combined.trim()) return null;
  for (const [type, pattern] of PROJECT_TYPE_PATTERNS) {
