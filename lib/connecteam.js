...
/** Shifts in [startTime, endTime), unix seconds. */
 ...
 * the day so repeated calls share the listShifts cache entry instead of each
 * paging the API from scratch.
 */
async function fetchProgressWindow(referenceUnixSeconds) {
  const windowDays = 180;
  const dayStart = Math.floor(referenceUnixSeconds / 86400) * 86400;
  return listShifts(dayStart - windowDays * 86400, dayStart + windowDays * 86400);
}
...
async function getJobProgress(title, referenceUnixSeconds = Math.floor(Date.now() / 1000)) {
  ...
  const shifts = await fetchProgressWindow(referenceUnixSeconds);
  const jobShifts = shifts.filter((s) => (s.title || '').trim().toLowerCase() === needle);
  if (!jobShifts.length) return null;

  return summarizeJobShifts(jobShifts, referenceUnixSeconds);
}
...
async function getJobProgressBulk(titles, referenceUnixSeconds = Math.floor(Date.now() / 1000)) {
  ...
  if (!needles.length) return new Map();
  ...
  for (const needle of needles) {
    ...
    if (jobShifts?.length) out.set(needle, summarizeJobShifts(jobShifts, referenceUnixSeconds));
  }
  return out;
}
